...
const PlaylistPage = () => {
        ...
        <List component='nav'>
            ...
            {playlist.tracks &&
                playlist.tracks.items.map((track: object) => (
                    <ItemList item={track.track} imageUrl={track.track.album?.images[0]?.url} artists={track.track.artists} />
                ))}
        </List>,
        <List component='nav'>
        <ShuffleButton body={{ 'uris': recommendedTracks.map(track => track.uri) }} />
    ...
    useEffect(() => {
        if (!playlist.tracks) {
            ...
            request('/recommendations?seed_tracks=' + sampleTracks)
